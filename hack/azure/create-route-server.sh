...
# And the same for the subnet, for the same reason. RouteServerSubnet is
# a fixed name Azure insists on, so one that is already there was put
# there by somebody, and a teardown that removes it regardless takes
# their subnet with it.
subnet_tag="bgp-cloud-connector-added-subnet-${infra}"
...
ensure_subnet() {
    ...
    if [[ -n "${prefix}" ]]; then
        # Adopted, not created, and deliberately not recorded: the
        # teardown leaves a subnet it has no record of creating.
        info "  adopting the existing ${rs_subnet} (${prefix}), so it is not ours"
        info "  (the teardown will leave it where it found it)"
        return 0
    fi
    ...
    # Recorded on the vnet, where the address prefix record already
    # lives, so the teardown can tell a subnet this cluster created from
    # one it merely found. A separate call because the subnet and the
    # tag live on different resources; a run interrupted between the two
    # leaves an untagged subnet, which the teardown then leaves alone --
    # the safe way round.
    try az network vnet update -g "${net_rg}" -n "${vnet}" \
        --set "tags.${subnet_tag}=${rs_subnet}" --output none
}
