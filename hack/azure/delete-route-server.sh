...
# And the same for the subnet. RouteServerSubnet is a fixed name Azure
# insists on, so one that was already there belongs to whoever put it
# there, and removing it takes whatever depended on it.
...
delete_subnet() {
    ...
    local owned
    owned="$(az_query "read the subnet record on ${vnet}" \
        az network vnet show -g "${net_rg}" -n "${vnet}" \
        --query "tags.\"${subnet_tag}\"" -o tsv)" \
        || { fail "read the subnet record on ${vnet}"; return 0; }
    [[ "${owned}" == "None" ]] && owned=""

    if [[ -z "${owned}" ]]; then
        info "  ${vnet} carries no record of a subnet created for ${infra}"
        info "  (nothing to remove; a subnet that was here first stays)"
        return 0
    fi
    ...
    if [[ -z "${prefix}" ]]; then
        # The record outlived the subnet, which is what a teardown
    ...
    az_retry "delete subnet ${rs_subnet}" "${delete_budget}" \
        ...
        || { fail "delete subnet ${rs_subnet}"; return 0; }

    # Only once the subnet is actually gone. The record outliving a
    # failed delete is what makes the next run try again.
    try az network vnet update -g "${net_rg}" -n "${vnet}" \
        --remove "tags.${subnet_tag}" --output none \
        || fail "clear the subnet record on ${vnet}"
}
